...
(Claude, with a deterministic stub fallback) only **explains** an already-classified variance in
...
flowchart LR
    ...
    RC --> VAR[(Variances)]
    VAR --> AI[AI Explanation Agent\nStub or Claude - narrates only, never computes]
...
├── agents/                   # AI explanation providers (StubExplanationProvider, ClaudeExplanationProvider)
...
|---|---|---|
| `ANTHROPIC_API_KEY` | No | If set, AI explanations use Claude (`ClaudeExplanationProvider`). If unset, a deterministic `StubExplanationProvider` generates rule-based explanation text — the app is fully functional without this key. |
...
| AI explanation button does nothing / errors | `ANTHROPIC_API_KEY` not set — this is fine, the stub provider should still respond | Check backend logs; if even the stub fails, it's likely an unrelated backend error, not a Claude API issue |
